(ns app.tutorial
  (:require
   [clojure.string :as string]
   [sci.impl.vars :refer [SciVar]]))

(def tutorial
  "Collection of map steps."
  [;; Strings
   {:title
    "안녕 문자열!"
    :content
    "> 원칙을 배우고, 원칙을 따르고, 원칙을 없애라. - 이소룡
     
  기본부터 시작해봅시다. 클로저는 정수, 부울린, 문자열과 같은 가장 기본이 되는 타입들을 제공합니다. 문자열은 `\"`로 감쌉니다.
    
   *Hello World*란 인삿말 문자열을 만들어 보세요`\"Hello World\"`"
    :test #(= (string/lower-case %) "hello world")}

   ;; Lists
   {:title "리스트를 이해하자"
    :content
    "> 리스프는 역사상 가장 위대한 프로그래밍 언어이다. - 앨런 케이
     
   리스프는 *리스트 프로세싱*을 나타내며,  특징으로 [리스트](https://en.wikipedia.org/wiki/S-expression)로 코드를 작성한다는 것입니다. 클로저는 리스프의 한 종류이기에 비슷한 문법을 사용합니다.
     
   리스트는 `(1 2 3)` 처럼 아이템들의 모음입니다.
    클로저에선 `list` 를 이용하거나, `'` 를 앞에붙여 리스트를 만들 수 있습니다.

   이제 리스트를 만들어 봅시다. 예를들어 `(list 1 2 3)` 아니면  `'(1 2 3)`."
    :test #(list? %)}

   ;; Math
   {:title "수학은 (즐거움)"
    :content
    "클로저에서 수학 연산자는 일반 함수와 같습니다.
   앞서 봐온것과 같이, 괄호 `(...)` 안에 넣어야 합니다.
  
  따라서, `4 + 2`대신에 `(+ 4 2)`라고 해야합니다. `+ - / *`를 이용해서 사칙연산을 수행해 보세요." 
    :test #(number? %)}

   ;; Functions
   {:title "클로저는 함수형입니다"
    :content
    "> 리스프는 함수형입니다. 그리고 함수형이 미래로 보입니다. - 로버트 C. 마틴
  
  리스트의 첫번째 인자는 **함수**이며, **나머지들은** 함수의 인자들입니다.
     `(not true)`에서 *not*은 부정하는 함수이고, *true*는 인자입니다.

   여러분의 이름을 \"문자열\" 인자로 넣어 함수 `(my-name)`을 사용해보세요. 예를들어 `(my-name \"Elia\")`"
    :test #(and (map? %) (contains? % :my-name) (string? (:my-name %)))}

   ;; Keywords
   {:title "키를 까먹지 마세요"
    :content
    "> 요즘날의 문제는 어떻게 혁신할 것인가가 아니다; 이미 존재하는 좋은 아이디어를 어떻게 사회가 받아들일 것인가이다. - 더글러스 엔겔바트
     
  안녕하세요 **[[my-name]]**씨! 만나서 반갑습니다.
    
  REPL에서 여러분이 입력한 표현식의 평가 결과얻어 왔습니다.
    여기서, `:my-name`은 특별한 폼(form)으로, *키워드*라고 합니다.
    키워드를 만들고 싶으면 만들고 싶은 단어 앞에 `:`를 붙여야 합니다.
     
  `:next`이라는 키워드를 이용해서 계속나아가세요."
    :test #(= % :next)}

  ;; Exercise - 01
   {:title "모든것을 위한 함수"
    :content
    "인터페이스를 고쳐봅시다!
     
  이제 어떻게 함수를 호출하는지, 키워드와 문자열을 어떻게 사용하는지 알게 되었습니다.
     다음으로 프롬프트를 변경해보면 어떨까요?

  `(create-dog :name \"Zeus\" :breed \"Beagle\")`와 같이 여러분은 키워드 파라미터를 이용해서 함수를 호출할 수 있습니다. 

  `(set-prompt)` 함수를 이용해서 색깔을 바꿔보세요. `:color`와 `:text`를 이용하여 문자열로 넣을 수 있습니다.
     잘 안되겠으면 `(set-prompt :color \"red\")`를 클릭하세요"
    :test #(contains? % :prompt-color)}
   ;; 
   {:title "함수형 견습생!"
    :content
    "축하합니다. 함수를 호출하여 애플리케이션의 상태를 변경했습니다!
     그리고 모든 명령어들은 리스트로 이루어져 있었습니다!
                                                                                     
  클로저는 `reverse`와 같이 리스트를 다루는 여러 함수들을 제공합니다. 이 함수는 컬렉션을 뒤집어 줍니다.
     따라서, 문자열을 넣으면 문자들이 들어있는 컬렉션으로 인식하게 됩니다.
     
  `(reverse \"a-long-string\")`을 입력하여 다음 단계로 나아가 봅시다."
    :test #(= % (reverse "a-long-string"))}
   
   ;; Vectors
   {:title "벡터를 가지게 되었습니다"
    :content
    "> 리스프를 배움으로써 얻게될 심오한 깨달음의 경험 때문이라도 리스프는 배울 가치가 있다. - 에릭 레이먼드
     
  **벡터**는 연속된 엘리먼트로 구성되어있으며, 리스트보다 빠른 접근이 가능합니다.
     
  벡터를 만들기 위해서는 사각 괄호 `[ ]` 안에 구분자 없이 아이템들을 넣어야 합니다.
     
  `[\"luna\" \"milu\" \"boris\"]`과 같이 여러분이 좋아하는 고양이 이름들이 들어있는 벡터를 만들어 봅시다."
    :test #(vector? %)}
   ;; Variables
   {:title "변수를 Def 하라"
    :content
    "> 좋은 프로그래머는 프로그램만 작성하지는 않습니다. 작업에 필요한 어휘도 구축합니다. - 가이 스틸
                                                                                     
  `def`를 이용하여 **전역** 변수를 정의합니다. 그 변수는 어떠한 것도 될 수 있습니다.
     
  `foo`라는 이름의 전역 변수를 만들어 보세요. 예) `(def foo \"bar\")`"
    :test #(and (instance? SciVar %) (= "foo" (-> (.-meta %) :name str)))}
   
   ;; Let
   {:title "Let it be local"
    :content
    "> 리스프는 광신도들에게만 보이는 마법과도 같은거라서 위대한것이 아니라, 그냥 그게 가장 강력한 언어이기 때문에 위대하다. - 폴 그레이엄
     
  **지역**변수는 `let`을 이용하여 정의할 수 있습니다.
   `let`의 어휘적 영역(lexical context)안에서만 유효합니다.
   `(let [x 1] x)`의 표현식에서, `x`를 참조하고 싶다면 `( )`로 둘러싸인 `영역`에서만 할 수 있습니다.
     
  `(let [a 2 b 3] (* a b))`와 같이, 숫자 변수를 만들고 곱해보세요."
    :test #(number? %)}
   ;; Maps
   {:title "맵(Map)은 사전(dictionary))"
    :content
    "> 컴퓨터가 이해할 수 있는 코드를 작성하는건 어떠한 바보라도 할 수 있다. 좋은 프로그래머는 사람이 이해할 수 있는 코드를 작성한다. - 마틴 파울러
     
  맵은 *키*와 *값*을 매핑하는 컬렉션입니다. `{ }`로 감싸져 있습니다.
     키로 아무거나 사용할 수 있지만, 클로저 프로그래머들은 주로 키워드를 사용합니다.

  `{:country \"Korea\"}`와 같이 `:country`를 키로 하고, 가고싶은 국가를 문자열로 하는 맵을 만들어 보세요."
    :test #(and (map? %) (contains? % :country) (string? (:country %)))}
   
   ;; F-list
   {:title "리스트의 처음(First)"
    :content
    "클로저는 리스트에서 값을 추출하기 위한 몇가지 함수를 제공합니다. 예를 들어, `first`는 첫번째 요소를 반환합니다.

  `(first '(\"alpha\" \"bravo\" \"charlie\"))`를 입력해서 첫번째 요소를 얻어보세요."
    :test #(and (string? %) (= "alpha" %))}
   
   ;; Range
   {:title "N의 범위(Range)"
    :content
    "클로저 함수 `range`는 0부터 `n`하나 전까지의 숫자 리스트를 만듭니다. 
     즉, `(range 5)`는 0부터 4까지의 숫자를 반환합니다. `(doc range)`를 입력해서 문서를 확인해보세요.
     
  0부터 99까지의 범위를 구해보세요. 타이핑이 귀찮으시다면 `(range 100)`를 클릭하세요 ^^"
    :test #(= % (range 100))}
   
   ;; Filter
   {:title "필터(Filter) 리스트"
    :content
    "리스트에 함수를 적용할 수 있습니다. 예를 들어, `filter`를 이용해서 조건에 맞는 요소들만 추릴 수 있습니다.
     
  0부터 50까지의 숫자에서 *짝수*만 제거해보세요.
  `odd?`는 홀수인지 판별하는 함수입니다. `even?`는 짝수인지 판별하는 함수입니다.
  타이핑이 귀찮으시다면, `(filter odd? (range 50))`를 클릭하세요 ^^"
    :test #(= % (filter odd? (range 50)))}
   
   ;; Map
   {:title "리스트에 함수 적용시키기"
    :content
    "100보다 작고 11의 배수인 숫자들을 찾기 위해서는, `5 * 10 + 5 = 55`와 같이 1부터 9까지의 각각의 숫자에 10을 곱하고, 숫자를 더하면 됩니다.
      클로저에서는 `map`을 이용해서 위와 같은 작업을 할 수 있습니다.

  `map`는 리스트의 모든 요소에 함수를 적용합니다.

  한번 `(map (fn [n] (+ n (* n 10))) (range 1 10))`를 이용해서 작업을 완료해보세요."
    :test #(= % (map (fn [n] (+ n (* n 10))) (range 1 10)))}
   
    ;; Inline functions
   {:title "인라인 함수"
    :content
    "이전 단계에서 이미, `map`에 인라인 함수 `(fn [n] (+ n (* n 10)))`를 작성하고 인자로 넘겼습니다.
   이 기법은 목적이 따로 없는 *일반적인 유틸리티* 함수를 만들 때 유용합니다.
     
  `넓이 w`와 `높이 h`를 인자로 받아 사각형의 둘레를 반환하는 인라인 함수를 만들어 봅시다:
      
  `(fn [w h] (* (+ w h) 2))`."
    :test #(= (apply % [2 3]) 10)}
   
   ;; REPL
   {:title "REPL 주도 개발"
    :content
    "> 새로운 프로그래밍 언어를 배우는 유일한 방법은, 그 언어로 프로그램을 작성하는 것이다. - 커니핸(Kernighan) 그리고 리치(Ritchie)

   이제까지 REPL에서 코드를 입력하고 테스트하는 방식으로 문제를 해결해 왔습니다.
     바로 클로저 인들이 하는 것처럼 말이죠! 컴파일하고 나중에 디버깅하는 것보다, 바로바로 코드를 입력하면서 테스트하는 것이 더 빠릅니다!

   이제까지 나왔던 것들보다 클로저로는 더 많을것들을 할 수 있습니다. `(more)`를 입력해서 마지막 단계로 가 보도록 하겠습니다."
    :test #(true? %)}

   {:title "Clojure를 배울 시간이 되었습니다!"
    :content
    "> 초심자의 마음에는 많은 가능성이 있지만, 숙련자의 마음에는 거의 없다. - 초심자의 마음, 젠 마인드
      
  보신것처럼 클로저는 어렵지 **않습니다**.
  괄호, 함수, 불변 데이터구조 그리고 REPL이 여러분의 친구가 될 것입니다.
  초심자의 마음을 잃지 마세요!
     
  [Clojure koans](http://clojurekoans.com/), [4Clojure](https://4clojure.oxal.org/) 또는 [exercism](https://exercism.org/tracks/clojure)는 좋은 시작점이 될것입니다.

  필요하다면 [커뮤니티에 도움](https://lispkorea.github.io/help/)을 요청해보세요, 행운을 빕니다!"
    :test #(true? false)}])
